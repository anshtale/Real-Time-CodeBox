...
export const CodeOutput = ({ output, onClear, input, onInputChange }: CodeOutputProps) => {
  return (
    <div className="space-y-4 h-full flex flex-col">
      ...
      {/* Output Section */}
      <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl p-4 shadow-xl flex-1 flex flex-col">
        ...
        <div className="flex-1 bg-gray-900/50 border border-gray-700 rounded-lg p-4 overflow-y-auto custom-scrollbar">
          {output.length > 0 ? (
            <div className="space-y-1">
              {output.map((line, index) => (
                <pre key={index} className="text-green-400 text-sm whitespace-pre-wrap font-mono">
                  {line}
                </pre>
              ))}
            </div>
